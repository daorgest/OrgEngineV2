...
void Platform::Init(WindowContext* window, i32 width, i32 height, DisplayMode mode)
{
	...
	if (!SDL_GetCurrentDisplayMode(displayID))
	{
		// fallback
		displayMode.w = 1280;
		displayMode.h = 720;
	...
	window->monitorWidth = displayMode.w;
	window->monitorHeight = displayMode.h;

	window->windowWidth = (width == 0) ? displayMode.w : width;
	window->windowHeight = (height == 0) ? displayMode.h : height;

	// Keep centered
	window->positionX = SDL_WINDOWPOS_CENTERED;
	window->positionY = SDL_WINDOWPOS_CENTERED;
	...
	Uint64 flags = SDL_WINDOW_RESIZABLE | SDL_WINDOW_VULKAN;
	...
	if (!win)
	{
		fmt::print(stderr, "[SDL3] SDL_CreateWindow FAILED: {}\n", SDL_GetError());
		std::abort();
	}
...
void Platform::LockCursor(WindowContext& window, bool lock)
{
	auto* win = static_cast<SDL_Window*>(window.handle);
	if (!win) return;
