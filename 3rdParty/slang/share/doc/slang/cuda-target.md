...
`WaveGetLaneIndex` computes the lane index by linearizing the thread index across all dimensions and masking to the warp
size:
...
This handles multi-dimensional thread blocks correctly as long as the linearized thread index maps to consecutive warp
lanes, which is the standard CUDA thread-to-lane mapping.

Alternatively, defining `SLANG_USE_ASM_LANE_ID` before including the CUDA prelude switches to an inline PTX assembly
implementation (`mov.u32 %0, %laneid`). The assembly version is always correct regardless of launch configuration, but
is slower than the arithmetic default.
