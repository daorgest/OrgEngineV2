...
Unlike C/C++, named `enum` types in Slang are always scoped by default (like `enum class` in C++), whereas anonymous
`enum` types are unscoped. You can also write `enum class` explicitly in Slang for clarity, but it isn't required. If
you want an `enum` type to be unscoped, you can use the `[UnscopedEnum]` attribute:
...
You can also use the `-unscoped-enum` command-line option to make all named `enum` types other than `enum class`
unscoped by default. It is an error to combine `[UnscopedEnum]` with `enum class`, or to declare an anonymous
`enum class`.
...
`m._41_32`. Matrix swizzle l-values may be assigned, and constant-indexed subscripts of matrix swizzles may be used
where an l-value is required.
