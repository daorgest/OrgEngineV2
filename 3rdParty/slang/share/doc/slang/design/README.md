...
The [Shader Coverage](shader-coverage.md) document describes the current architecture for shader coverage
instrumentation, including the interaction between IR-time buffer synthesis and post-emit metadata.

The [Shader Coverage Counter Placement](shader-coverage-counter-placement.md) document describes where line, function,
and branch coverage counters are inserted.
